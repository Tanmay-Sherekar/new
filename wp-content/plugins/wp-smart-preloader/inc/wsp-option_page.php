	...
	*/
	...
	public function wsp_preloader_list(){
		...
		foreach($loader as $load){
			$sel = ( isset( $this->options['loader'] ) && esc_attr($this->options['loader']) == $load ) ? 'selected="selected"' : '';
	...
	public function wsp_preloader_homepage(){
		printf(__('<input type="checkbox" name="%s" value="1" %s />','wp-smart-preloader'),"wsp-loader-opt[homepage]",(isset($this->options['homepage']) && esc_attr($this->options['homepage']) == "1")?'checked="checked"':'' );
	}
	...
	public function wsp_loader_fadeOut(){
		printf(__('<input type="text" name="%s" value="%s"  /><div>%s</div>','wp-smart-preloader'),"wsp-loader-opt[fadeout]",(isset($this->options['fadeout']) )?esc_attr($this->options['fadeout']):'',__('Default:2500<br />1 second => 1000',"wp-smart-preloader") );
	}
	...
	public function wsp_loader_delay(){
		printf(__('<input type="text" name="%s" value="%s"  /><div>%s</div>','wp-smart-preloader'),"wsp-loader-opt[delay]",(isset($this->options['delay']) )?esc_attr($this->options['delay']):'',__('Default:1500<br />1 second => 1000',"wp-smart-preloader") );
	}
